...
class unet_less_layers_single_conv(nn.Layer):
    def __init__(self,
                 num_classes,
                 in_channels=3):
        super().__init__()
...
class Encoder(nn.Layer):
    def __init__(self, in_channels=3):
        ...
        self.single_conv = layers.ConvBNReLU(in_channels, 64, 3)
...
class UpSampling(nn.Layer):
    def __init__(self,
        ...
        self.up_conv_2x2 = nn.Conv2D(in_channels=in_channels, out_channels=out_channels, kernel_size=2, padding='same')

        self.single_conv = layers.ConvBNReLU(in_channels, out_channels, 3)

    def forward(self, x, short_cut):
        x = self.up_conv_2x2(x)
        x = F.interpolate(
            x,
